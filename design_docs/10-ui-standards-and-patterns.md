...
**Enforcement — the token guard (`eslint.config.js`):** `npm run lint` fails on color that bypasses the token layer. Three shapes are caught, because color has escaped through all three:

- **Named palette classes** — `text-gray-500`, `border-red-200`. The original rule; matches only shade-numbered classes, so `text-[var(--color-text-muted)]` is never flagged.
- **Arbitrary hex values** — `bg-[#B7102A]`. This was the blind spot that let the Rubric tools accumulate ~180 hardcoded Bauhaus hexes undetected, since they are not shade-numbered.
- **Bare hex literals** — `style={{ borderColor: '#d1d5db' }}`, `const RED = '#B7102A'`. Pinned to 3/4/6/8-digit hex so DOM ids and href anchors do not trip it.

The **one sanctioned exception** is markup injected into a Canvas page (`src/content_scripts/`): it renders outside our cascade, where none of our custom properties exist, so it needs literal values. Those sites carry an inline `eslint-disable-next-line no-restricted-syntax` naming that reason. Nothing inside the extension's own pages qualifies.

`white` is deliberately **not** guarded yet — it means two different things (foreground on an accent fill, which wants `--primary-contrast`; and a surface fill, which wants a surface token) and needs a decision before a rule. Tracked as `canvas-power-tools-c00`.

**Pattern for Bauhaus-only visual mechanics:** several rules in this doc (domain accent strips, data-table header/toolbar treatment, bordered badges) apply under Bauhaus only and are explicitly a no-op under Modern. These are implemented the same way each time — a base class in `global.css` with an inert default (`border-width: 0`, `background: transparent`, etc.), then a `[data-theme="bauhaus"] .the-class { ... }` override that turns it on. See `.domain-accent`, `.table-header-cell`, `.table-toolbar`, `.badge-pill`. Per-tool rebuilds should reuse this pattern rather than branching on theme in JS.
...
- **Palette:** `--color-cat-1` … `--color-cat-8` in `global.css`. Eight hues, theme-independent (unlike Bauhaus mechanics, category color coding stays on under Modern too).
...
- Status cells: `Badge` atom + `.badge-pill` class — flat `--radius-sm` + 1px `currentColor` border under Bauhaus (soft tinted pill, no border, under Modern). Used for Published/Unpublished in `AssignmentTable`.
...
- Sidebar row selection: solid `--cpt-color` fill + white text on the selected row (both themes) rather than a Bauhaus-only mechanic — a single-select nav list needs the same affordance under Modern as under Bauhaus, so this isn't gated behind `[data-theme="bauhaus"]`.
