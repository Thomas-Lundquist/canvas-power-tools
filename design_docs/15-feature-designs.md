...
The group's header toolbar is a "Rename" button plus two menus (using the shared `Menu` component) to avoid crowding: a "Sort" menu (per above) and an "Actions" menu holding Duplicate, Copy to, Merge, "Bulk edit assignments…", and Delete Group.

**Group color.** A `GroupColorSwatch` sits at the left of each group's header, between the expand toggle and the name — the filled square doubles as the always-on indicator and the picker (click → portaled popover of the 8 `--color-cat-*` swatches). Picks are a display-only local preference (`chrome.storage.sync`, `src/storage/groupColors.js`) — never a Canvas write, so no PIN gate. Unpicked groups auto-take a distinct color by group order. The same color shows against each assignment in the Bulk Edit Tool's Group column (see Doc 10 § Categorical Color Coding).

### Boundary with the Bulk Edit Tool

This tool owns the *group* as an object — group lifecycle (create, rename, weight, reorder, duplicate, merge, delete) and single-assignment nudges made while browsing a group (reorder within a group, the per-row "move to group" dropdown).

Every *multi-assignment* operation — bulk group moves, bulk delete, and bulk date / point / publish edits — belongs to the Bulk Edit Tool, which already owns selection, filtering, preview, the Change Log, and per-assignment failure handling. Duplicating that machinery here (a scoped checklist modal per group) was a mistake and has been removed. Instead, **"Bulk edit assignments…"** (Actions menu) is a deep link: it opens the Bulk Edit Tool at `src/pages/bulk-editor/index.html?courseId=&group=&selectAll=1`, pre-filtered to this group with every assignment in it pre-selected, ready to move, delete, or edit. See Doc 02.
