...
| 2 | **`publish_at` is out of scope — the feature is not enabled on this instance.** | The Canvas docs state it *"will have no effect unless the 'Scheduled Page Publication' feature is enabled in the account."* **Checked 2026-09-21:** `GET /api/v1/courses/:id/features/enabled` returns 23 flags and `scheduled_page_publication` is not among them, so the field would be accepted and silently ignored. A control that does nothing is worse than no control, so scheduling stays out. Turning it on is **an admin action, not a teacher one**: `GET /api/v1/courses/:id/features` lists the 7 course-scoped flags a teacher can see, and this is not one of them. So this is an account setting, not a permanent fact — if an admin ever switches it on, `publish_at` becomes a clean addition, but the surprising interaction below then has to be surfaced in the UI at the point of action, which is design work on top of the API work. Re-check with either endpoint. |
...
1. ~~**Is "Scheduled Page Publication" enabled in the target Canvas instance?**~~ **Answered 2026-09-21: no.** The flag is absent from `GET /api/v1/courses/:id/features/enabled`, so `publish_at` is not buildable here and Decision 2 is settled rather than pending. Caveat on the evidence: that endpoint lists the flags that apply to a course, so an account-only flag could in principle be enabled without appearing. If certainty is ever needed, set `publish_at` on a scratch page and see whether it sticks.
